...
RUN apk --no-cache update && apk add --no-cache openssl tzdata \
    && mkdir /pwmgr \
    && mkdir /db
...
# run:
# docker run --name pwmgr-server --restart unless-stopped -d -v /path/to/db-store:/db -v /etc/localtime:/etc/localtime:ro -p 48222:48222 pwmgr-server
